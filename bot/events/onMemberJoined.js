...
module.exports = async ( client, member ) => {
  ...
  let pickMessage     = discordJoinMessages[ Math.floor( Math.random() * Math.floor( discordJoinMessages.length ) ) ];
  // replace the "user" by the member's name
  let welcomeMessage  = pickMessage.replace( '{{user}}', member.user.tag );
  // get the guild's welcome channel and return the function if it didn't find any
